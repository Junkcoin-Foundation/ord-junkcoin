...
impl Context {
  ...
  #[track_caller]
  pub(crate) fn assert_junes(
    &self,
    mut junes: impl AsMut<[(JuneId, JuneEntry)]>,
    mut balances: impl AsMut<[(OutPoint, Vec<(JuneId, u128)>)]>,
  ) {
    let junes = junes.as_mut();
    junes.sort_by_key(|(id, _)| *id);
    ...
    assert_eq!(junes, self.index.junes().unwrap());

    assert_eq!(balances, self.index.get_june_balances());

    let mut outstanding: HashMap<JuneId, u128> = HashMap::new();
