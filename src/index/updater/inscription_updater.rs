use crate::junk20::operation::{Action, InscriptionOp};
      ...
      Origin::New {
        ...
        self.id_to_entry.insert(
          ...
          &InscriptionEntry {
            fee,
            height: self.height,
            inscription_number: if self.height >= self.chain.first_inscription_height() {
              self.number_to_id.insert(&self.next_number, &inscription_id)?;
              self.next_number += 1;
              self.next_number - 1
            } else {
              u64::MAX // Special value for inscriptions before first_inscription_height
            },
            sat,
            sequence_number: 0,
