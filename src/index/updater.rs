...
use {
  self::{june_updater::JuneUpdater, inscription_updater::InscriptionUpdater},
...
use crate::junk20::BlockContext;
use crate::index::updater::junk20_updater::Junk20Updater;
use crate::sat::Sat;
...
impl<'index> Updater<'_> {
  ...
  fn index_block(
    ...
    let mut junk20_token_info = wtx.open_table(JUNK20_TOKEN)?;
    let mut junk20_token_holder = wtx.open_multimap_table(JUNK20_TOKEN_HOLDER)?;
    let mut junk20_token_balance = wtx.open_table(JUNK20_BALANCES)?;
    let mut junk20_inscribe_transfer = wtx.open_table(JUNK20_INSCRIBE_TRANSFER)?;
    let mut junk20_transferable_log = wtx.open_table(JUNK20_TRANSFERABLELOG)?;
...

    {
      ...
      if index.index_junk20 && self.height >= index.first_junk20_height {
        ...
        Junk20Updater::new(
          &mut junk20_token_info,
          &mut junk20_token_holder,
          &mut junk20_token_balance,
          &mut junk20_inscribe_transfer,
          &mut junk20_transferable_log,
    ...
    if index.index_junes && self.height >= self.index.first_june_height {
      let mut outpoint_to_june_balances = wtx.open_table(OUTPOINT_TO_JUNE_BALANCES)?;
      let mut june_id_to_june_entry = wtx.open_table(JUNE_ID_TO_JUNE_ENTRY)?;
      let mut june_to_june_id = wtx.open_table(JUNE_TO_JUNE_ID)?;
      let mut inscription_id_to_june = wtx.open_table(INSCRIPTION_ID_TO_JUNE)?;
      let mut june_updater = JuneUpdater::new(
        self.height,
        &mut outpoint_to_june_balances,
        &mut june_id_to_june_entry,
        &inscription_id_to_inscription_entry,
        &mut inscription_id_to_june,
        &mut june_to_june_id,
        &mut statistic_to_count,
        block.header.time,
        June::minimum_at_height(Chain::Mainnet, Height(self.height)),
      )?;
      for (i, (tx, txid)) in block.txdata.iter().enumerate() {
        june_updater.index_junes(i, tx, *txid)?;
      }
    }
