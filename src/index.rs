use {
  self::{
    junes::{June, JuneId},
    entry::{
      BlockHashValue, JuneEntryValue, JuneIdValue, Entry, InscriptionEntry, InscriptionEntryValue,
...
use crate::junk20::{Balance, max_script_tick_key, min_script_tick_key, script_tick_key, Tick, TokenInfo, TransferableLog, min_script_tick_id_key, max_script_tick_id_key};
use crate::junk20::script_key::ScriptKey;
use crate::sat::Sat;
use crate::sat_point::SatPoint;
use crate::templates::BlockHashAndConfirmations;

pub(crate) use self::entry::JuneEntry;
...
define_table! { INSCRIPTION_ID_TO_JUNE, &InscriptionIdValue, u128 }
define_table! { INSCRIPTION_ID_TO_SATPOINT, &InscriptionIdValue, &SatPointValue }
define_table! { INSCRIPTION_NUMBER_TO_INSCRIPTION_ID, u64, &InscriptionIdValue }
define_table! { OUTPOINT_TO_JUNE_BALANCES, &OutPointValue, &[u8] }
...
define_table! { JUNE_ID_TO_JUNE_ENTRY, JuneIdValue, JuneEntryValue }
define_table! { JUNE_TO_JUNE_ID, u128, JuneIdValue }
...
define_table! { TRANSACTION_ID_TO_JUNE, &TxidValue, u128 }
define_table! { TRANSACTION_ID_TO_TRANSACTION, &TxidValue, &[u8] }
define_table! { WRITE_TRANSACTION_STARTING_BLOCK_COUNT_TO_TIMESTAMP, u32, u128 }
define_table! { JUNK20_BALANCES, &str, &[u8] }
define_table! { JUNK20_TOKEN, &str, &[u8] }
define_table! { JUNK20_INSCRIBE_TRANSFER, &InscriptionIdValue, &[u8] }
define_table! { JUNK20_TRANSFERABLELOG, &str, &[u8] }
define_multimap_table! { JUNK20_TOKEN_HOLDER, &str, &str}
...
pub(crate) struct Index {
  ...
  first_june_height: u32,
  first_junk20_height: u32,
...
impl Index {
  pub(crate) fn open(options: &Options) -> Result<Self> {
    ...
    let database = match unsafe { Database::builder().open(&path) } {
      Ok(database) => {
        {
          ...
          index_junes = statistics
            .get(&Statistic::IndexJunes.key())?
            .unwrap()
            .value()
            != 0;
          ...
          index_transactions = statistics
            ...
            .value()
            != 0;
          index_junk20 = statistics
            .get(&Statistic::IndexJunk20.key())?
            .unwrap()
            .value()
            != 0;
        }
        ...
        if error.kind() == io::ErrorKind::NotFound =>
      {
        ...
        tx.open_table(INSCRIPTION_ID_TO_JUNE)?;
...

        {
          ...
          index_junk20 = options.index_junes();
          index_junes = options.index_junes();
          index_sats = options.index_sats;
          index_transactions = options.index_transactions;

          statistics.insert(&Statistic::IndexJunk20.key(), &u64::from(index_junk20))?;

          statistics.insert(&Statistic::IndexJunes.key(), &u64::from(index_junes))?;
    ...
    Ok(Self {
      ...
      first_june_height: options.first_june_height(),
      first_junk20_height: options.first_junk20_height(),
  ...
  pub(crate) fn has_june_index(&self) -> bool {
    self.index_junes
  }
  ...
  pub(crate) fn get_june_by_id(&self, id: JuneId) -> Result<Option<June>> {
    Ok(
      self
        .database
        .begin_read()?
        .open_table(JUNE_ID_TO_JUNE_ENTRY)?
        .get(&id.store())?
        .map(|entry| JuneEntry::load(entry.value()).june),
    )
  }

  pub(crate) fn june(&self, june: June) -> Result<Option<(JuneId, JuneEntry)>> {
    let rtx = self.database.begin_read()?;

    let entry = match rtx.open_table(JUNE_TO_JUNE_ID)?.get(june.0)? {
      Some(id) => rtx
        .open_table(JUNE_ID_TO_JUNE_ENTRY)?
        .get(id.value())?
        .map(|entry| (JuneId::load(id.value()), JuneEntry::load(entry.value()))),
      None => None,
    };
  ...
  pub(crate) fn junes(&self) -> Result<Vec<(JuneId, JuneEntry)>> {
    ...
    {
      let (id, entry) = result?;
      entries.push((JuneId::load(id.value()), JuneEntry::load(entry.value())));
    }

    Ok(entries)
  }

  pub(crate) fn get_june_balance(&self, outpoint: OutPoint, id: JuneId) -> Result<u128> {
    if self.block_count()? >= self.first_june_height && self.index_junes {
      let rtx = self.database.begin_read()?;

      let outpoint_to_balances = rtx.open_table(OUTPOINT_TO_JUNE_BALANCES)?;
      ...
      while i < balances_buffer.len() {
        let (balance_id, length) = junes::varint::decode(&balances_buffer[i..]);
        i += length;
        let (amount, length) = junes::varint::decode(&balances_buffer[i..]);
        i += length;

        if JuneId::try_from(balance_id).unwrap() == id {
          return Ok(amount);
        }
      }
    }
  ...
  pub(crate) fn get_june_balances_for_outpoint(
    &self,
    outpoint: OutPoint,
  ) -> Result<Vec<(SpacedJune, Pile)>> {
    if self.block_count()? >= self.first_june_height && self.index_junes {
      let rtx = &self.database.begin_read()?;

      let outpoint_to_balances = rtx.open_table(OUTPOINT_TO_JUNE_BALANCES)?;

      let id_to_june_entries = rtx.open_table(JUNE_ID_TO_JUNE_ENTRY)?;
      ...
      let mut i = 0;
      while i < balances_buffer.len() {
        let (id, length) = junes::varint::decode(&balances_buffer[i..]);
        i += length;
        let (amount, length) = junes::varint::decode(&balances_buffer[i..]);
        i += length;
        ...
        let entry = JuneEntry::load(id_to_june_entries.get(id.store())?.unwrap().value());
  ...
  pub(crate) fn get_june_balance_map(
    &self,
  ) -> Result<BTreeMap<SpacedJune, BTreeMap<OutPoint, u128>>> {
    let outpoint_balances = self.get_june_balances()?;

    let rtx = self.database.begin_read()?;

    let june_id_to_june_entry = rtx.open_table(JUNE_ID_TO_JUNE_ENTRY)?;

    let mut june_balances: BTreeMap<SpacedJune, BTreeMap<OutPoint, u128>> = BTreeMap::new();

    for (outpoint, balances) in outpoint_balances {
      for (june_id, amount) in balances {
        let spaced_june = JuneEntry::load(
          june_id_to_june_entry
            .get(&june_id.store())?
            .unwrap()
            .value(),
        )
  ...
  pub(crate) fn get_june_balances(&self) -> Result<Vec<(OutPoint, Vec<(JuneId, u128)>)>> {
    ...
    if self.block_count()? >= self.first_june_height && self.index_junes {
      for entry in self
        .database
        .begin_read()?
        .open_table(OUTPOINT_TO_JUNE_BALANCES)?
        .iter()?
      {
        ...
        while i < balances_buffer.len() {
          let (id, length) = junes::varint::decode(&balances_buffer[i..]);
          i += length;
          let (balance, length) = junes::varint::decode(&balances_buffer[i..]);
          i += length;
          balances.push((JuneId::try_from(id)?, balance));
        }
  ...
  pub(crate) fn get_junk20_balances(&self, script_key: &ScriptKey) -> Result<Vec<Balance>> {
    if self.block_count().unwrap() >= self.first_inscription_height {
      let rtx = self.database.begin_read()?;

      let junk20_token_balance = rtx.open_table(JUNK20_BALANCES)?;

      return Ok(
        junk20_token_balance
          .range(
  ...
  pub(crate) fn get_junk20_balance(
    &self,
  ...
  pub(crate) fn get_junk20_token_info(&self, tick: &Tick) -> Result<Option<TokenInfo>> {
    if self.block_count().unwrap() >= self.first_inscription_height {
      let rtx = self.database.begin_read()?;

      let junk20_token_info = rtx.open_table(JUNK20_TOKEN)?;
      return Ok(
        junk20_token_info
  ...
  pub(crate) fn get_junk20_tokens_info(&self) -> Result<Vec<TokenInfo>> {
    if self.block_count().unwrap() >= self.first_inscription_height {
      let rtx = self.database.begin_read()?;
  ...
  pub(crate) fn get_junk20_token_holder(&self, tick: &Tick) -> Result<Vec<ScriptKey>> {
    if self.block_count().unwrap() >= self.first_inscription_height {
      let rtx = self.database.begin_read()?;

      let junk20_token_holder = rtx.open_multimap_table(JUNK20_TOKEN_HOLDER)?;
  ...
  pub(crate) fn get_junk20_transferable_by_range(
    &self,
    script: &ScriptKey,
  ) -> Result<Vec<TransferableLog>, redb::Error> {
    let rtx = self.database.begin_read()?;
    let junk20_transferable_log = rtx.open_table(JUNK20_TRANSFERABLELOG)?;
    let result = Ok(
      junk20_transferable_log
  ...
  pub(crate) fn get_junk20_transferable_by_tick(
    &self,
    script: &ScriptKey,
    tick: &Tick,
  ) -> Result<Vec<TransferableLog>, redb::Error> {
  ...
  pub(crate) fn get_junk20_transferable_by_id(
    &self,
    script_key: &ScriptKey,
    ...
    if self.block_count().unwrap() >= self.first_inscription_height {
      ...
      let transferable_log_vec: Vec<TransferableLog> = junk20_transferable_log
  ...
  pub(crate) fn get_etching(&self, txid: Txid) -> Result<Option<SpacedJune>> {
    if self.block_count().unwrap() >= self.first_june_height {
      let rtx = self.database.begin_read()?;

      let transaction_id_to_june = rtx.open_table(TRANSACTION_ID_TO_JUNE)?;
      let Some(june) = transaction_id_to_june.get(&txid.store())? else {
        return Ok(None);
      };

      let june_to_june_id = rtx.open_table(JUNE_TO_JUNE_ID)?;
      let id = june_to_june_id.get(june.value())?.unwrap();

      let june_id_to_june_entry = rtx.open_table(JUNE_ID_TO_JUNE_ENTRY)?;
      let entry = june_id_to_june_entry.get(&id.value())?.unwrap();

      Ok(Some(JuneEntry::load(entry.value()).spaced_june()))
    } else {
      Ok(None)
    }
  }
  ...
  pub(crate) fn get_june_by_inscription_id(
    &self,
    inscription_id: InscriptionId,
  ) -> Result<Option<SpacedJune>> {
    let rtx = self.database.begin_read()?;
    let Some(june) = rtx
      .open_table(INSCRIPTION_ID_TO_JUNE)?
      .get(&inscription_id.store())?
      .map(|entry| June(entry.value()))
    else {
      return Ok(None);
    };
    let june_to_june_id = rtx.open_table(JUNE_TO_JUNE_ID)?;
    let id = june_to_june_id.get(june.0)?.unwrap();
...
mod tests {
  ...
  fn inscriptions_below_first_inscription_height_are_skipped() {
    ...
    // Test inscription before first_inscription_height
    {
      ...
      // Inscription exists but has no number
      let entry = context
        .index
        .get_inscription_entry(inscription_id)
        .unwrap()
        .unwrap();
      assert_eq!(entry.inscription_number, u64::MAX);

      // Verify inscription is tracked but not numbered
      assert_eq!(
        context.index.get_inscription_by_id(inscription_id).unwrap(),
        Some(inscription.clone())
      );
    }

    // Test inscription at first_inscription_height
    {
      let context = Context::builder()
        .arg("--first-inscription-height=3")
        .build();
      context.mine_blocks(2); // Mine to height 2
      let txid = context.rpc_server.broadcast_tx(template.clone());
      let inscription_id = InscriptionId::from(txid);
      context.mine_blocks(1); // Mine to height 3

      // Verify inscription gets number 0
      let entry = context
        .index
        .get_inscription_entry(inscription_id)
        .unwrap()
    ...
    // Test inscription after first_inscription_height 
    {
      let context = Context::builder()
        .arg("--first-inscription-height=3")
        .build();
      context.mine_blocks(3); // Mine to height 3
      let txid = context.rpc_server.broadcast_tx(template);
      let inscription_id = InscriptionId::from(txid);
      context.mine_blocks(1); // Mine to height 4

      // Verify inscription gets number 1
      let entry = context
        .index
        .get_inscription_entry(inscription_id)
        .unwrap()
