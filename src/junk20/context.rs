...
use crate::junk20::operation::{Action, deserialize_junk20_operation, InscriptionOp, Operation};
use crate::junk20::transfer::Transfer;
use crate::junk20::TransferInfo;
...
impl Message {
  pub(crate) fn resolve<'a, 'db, 'tx>(
    junk20_inscribe_transfer: &'a mut Table<'db, 'tx, &'static InscriptionIdValue, &'static [u8]>,
    ...
    let junk20_operation = match op.action {
      Action::New { inscription: _ } if sat_in_outputs => {
        match deserialize_junk20_operation(
          new_inscriptions
          ...
          Ok(junk20_operation) => junk20_operation,
          _ => return Ok(None),
        }
      }
      // Transfered inscription operation.
      // Attempt to retrieve the `InscribeTransfer` Inscription information from the data store of JUNK20.
      Action::Transfer => {
        match get_inscribe_transfer_inscription(junk20_inscribe_transfer, op.inscription_id) {
