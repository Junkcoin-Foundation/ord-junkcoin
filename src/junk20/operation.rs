...
use crate::junk20::deploy::Deploy;
use crate::junk20::errors::JSONError;
use crate::junk20::mint::Mint;
use crate::junk20::OperationType;
use crate::junk20::params::PROTOCOL_LITERAL;
...
pub(crate) fn deserialize_junk20_operation(
  ...
  let raw_operation = match deserialize_junk20(content_body) {
    Ok(op) => op,
    Err(e) => {
      return Err(e.into());
    }
  ...
  match action {
    ...
    Action::Transfer => match raw_operation {
      ...
      _ => Err(JSONError::NotJUNK20Json.into()),
    },
  }
}

fn deserialize_junk20(s: &str) -> Result<RawOperation, JSONError> {
  ...
  if identifier != Some(&json!(PROTOCOL_LITERAL)) {
    return Err(JSONError::NotJUNK20Json);
  }
