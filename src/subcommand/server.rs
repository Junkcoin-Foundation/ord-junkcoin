...
use {
  ...
  crate::{
    ...
    templates::{
      ...
      RangeHtml, RareTxt, SatHtml, JunkscriptionJson, TransactionHtml, Utxo, JUNK20,
    },
  },
  axum::{
    body,
...
use crate::junk20::operation::{deserialize_junk20_operation, Action};
use crate::junk20::token_info::{ExtendedTokenInfo, HolderBalanceForTick, HoldersInfoForTick};
use crate::templates::{JUNK20Balance, JUNK20Output, JUNK20UtxoOutput};
...
#[derive(Deserialize)]
struct Junk20TickInfoQuery {
  show_holder: Option<bool>,
}

#[derive(Deserialize)]
struct Junk20BalanceQuery {
  show_all: Option<bool>,
...
#[derive(Deserialize)]
struct JunesBalanceQuery {
  show_all: Option<bool>,
  list_junes: Option<bool>,
  filter: Option<SpacedJune>,
}
...
impl Server {
  pub(crate) fn run(self, options: Options, index: Arc<Index>, handle: Handle) -> SubcommandResult {
    Runtime::new()?.block_on(async {
      ...
      let router = Router::new()
        ...
        .route("/june/:june", get(Self::june))
        .route("/junes", get(Self::junes))
        .route("/junes/balances", get(Self::junes_balances))
        .route(
          "/junes/balance/:address",
          get(Self::junes_by_address_unpaginated),
        )
        .route("/junes/balance/:address/:page", get(Self::junes_by_address))
        .route(
        ...
        .route("/junk20/tick/:tick", get(Self::junk20_tick_info))
        .route("/junk20/tick", get(Self::junk20_all_tick_info))
        .route(
            "/junk20/balance/:address",
            get(Self::junk20_by_address_unpaginated),
        )
        .route("/junk20/validate", get(Self::junk20_validate))
        .route("/junk20/ticks", get(Self::junk20_all_ticks))
        .route("/junk20/tick/holder/:tick", get(Self::junk20_tick_holder))
        .route("/junes_on_outputs", get(Self::junes_by_outputs))
  ...
  async fn output(
    ...
    let junes = index.get_june_balances_for_outpoint(outpoint)?;
  ...
  async fn get_utxos_by_address(
    ...
    for outpoint in outpoints {
      if !index.get_june_balances_for_outpoint(outpoint)?.is_empty() {
        continue;
      }
      if !show_all
  ...
  async fn junk20_by_address(
    Extension(index): Extension<Arc<Index>>,
    Path(params): Path<(String, u32)>,
    Query(query): Query<Junk20BalanceQuery>,
  ) -> ServerResult<Response> {
    Self::get_junk20_by_address(index, params.0, Some(params.1), query).await
  }

  async fn junk20_by_address_unpaginated(
    Extension(index): Extension<Arc<Index>>,
    Path(params): Path<String>,
    Query(query): Query<Junk20BalanceQuery>,
  ) -> ServerResult<Response> {
    Self::get_junk20_by_address(index, params, None, query).await
  }

  async fn get_junk20_by_address(
    index: Arc<Index>,
    address: String,
    page: Option<u32>,
    query: Junk20BalanceQuery,
  ) -> ServerResult<Response> {
    task::block_in_place(|| {
      ...
      let mut junk20_utxos: Vec<(JUNK20, Utxo, InscriptionId, u64, u64, bool)> = Vec::new();
      ...
      if show_utxos {
        ...
        for outpoint in outpoints {
          ...
          for inscription_id in inscriptions {
            ...
            if let Some(content) = str_content {
              let junk20 = JUNK20::from_json_string(content.as_str());
              if let Some(junk20) = junk20 {
                if let Some(filter) = query.tick.clone() {
                  if filter != junk20.clone().tick.unwrap_or_default() {
                    continue;
                  }
                }
                let txid = outpoint.txid;
                ...
                let output = index
                  .get_transaction(txid)?
                  .ok_or_not_found(|| format!("junes {txid} current transaction"))?
                  .output
                  .into_iter()
                  .nth(vout.try_into().unwrap())
                  .ok_or_not_found(|| format!("junes {vout} current transaction output"))?;
        ...
        if !inscription_ids_to_check.is_empty() {
          ...
          for (junk20, _, id, _, _, ref mut valid) in &mut junk20_utxos {
            if let Some(ref op) = junk20.op {
              if *op == Operation::Transfer {
      ...
      let mut junk20balances: Vec<JUNK20Balance> = Vec::new();

      let balance = index
        .get_junk20_balances(&ScriptKey::from_address(
          address_from_str,
          index.get_network()?,
        ))
        .map_err(|err| ServerError::BadRequest(err.to_string()))?;
      ...
      if !balance.is_empty() {
        for entry in balance {
          ...
          let utxos: Vec<JUNK20Output> = junk20_utxos
            .iter()
            ...
            .collect();
          let token_info = index.get_junk20_token_info(&entry.tick.clone())?;
          ...
          if let Some(junk20_balance) = JUNK20Balance::from_strings(
            tick.as_str(),
            ...
            junk20balances.push(junk20_balance);
          }
        }
      }
      Ok(Json(json!({"junk20": junk20balances})).into_response())
    })
  }
  ...
  async fn junes_by_address(
    Extension(index): Extension<Arc<Index>>,
    Path(params): Path<(String, u32)>,
    Query(query): Query<JunesBalanceQuery>,
  ) -> ServerResult<Response> {
    Self::get_junes_by_address(index, params.0, Some(params.1), query).await
  }

  async fn junes_by_address_unpaginated(
    Extension(index): Extension<Arc<Index>>,
    Path(params): Path<String>,
    Query(query): Query<JunesBalanceQuery>,
  ) -> ServerResult<Response> {
    Self::get_junes_by_address(index, params, None, query).await
  }

  async fn get_junes_by_address(
    index: Arc<Index>,
    address: String,
    page: Option<u32>,
    query: JunesBalanceQuery,
  ) -> ServerResult<Response> {
    let (address, page) = (address, page.unwrap_or(0));
    let show_all = query.show_all.unwrap_or(false);
    let list_junes = query.list_junes.unwrap_or(false);
    ...
    let mut june_balances_map: LinkedHashMap<SpacedJune, JuneBalance> = LinkedHashMap::new();

    for outpoint in outpoints {
      let junes = index.get_june_balances_for_outpoint(outpoint)?;
      for (june, balances) in junes {
        if let Some(filter) = query.filter {
          if june != filter {
            continue;
          }
        }
        let june_balance = june_balances_map
          .entry(june.clone())
          .or_insert_with(|| JuneBalance {
            june: june.clone(),
        ...
        if !list_junes {
          ...
          june_balance.balances.push(JuneOutput {
            txid,
            vout,
            script: output.script_pubkey,
            shibes: output.value,
        ...
        june_balance.total_balance += balances.amount;
        june_balance.total_outputs += 1;
        elements_counter += 1;
      }
    }

    let june_balances: Vec<JuneBalance> = if show_all {
      june_balances_map.values().cloned().collect()
    } else if list_junes {
      june_balances_map
        .values()
        .cloned()
      ...
      let values: Vec<JuneBalance> = june_balances_map.values().cloned().collect();
      let mut items_collected = 0;
      let mut result = Vec::new();
      for value in values.iter() {
        let balances: Vec<JuneOutput> = value
          .balances
          .iter()
          .skip(start_index)
        ...
        result.push(JuneBalance {
          june: value.june.clone(),
    ...
    Ok(
      Json(JuneAddressJson {
        junes: june_balances,
        total_junes: june_balances_map.len(),
  ...
  async fn junk20_tick_info(
    Extension(index): Extension<Arc<Index>>,
    Path(tick): Path<String>,
    Query(query): Query<Junk20TickInfoQuery>,
  ) -> Result<Response, ServerError> {
    let tick =
      &Tick::from_str(tick.as_str()).map_err(|err| ServerError::BadRequest(err.to_string()))?;
    let token_info = index.get_junk20_token_info(&tick.clone())?;

    if query.show_holder.unwrap_or(false) {
      let holder = index.get_junk20_token_holder(&tick.clone())?;
      ...
      for script_key in holder.clone() {
        if let Some(balance) = index
          .get_junk20_balance(&script_key, &tick)
          .map_err(|err| ServerError::BadRequest(err.to_string()))?
        {
          let token_info_clone = token_info.clone().unwrap();
          let decimals = token_info_clone.decimal;
  ...
  async fn junk20_all_tick_info(
    Extension(index): Extension<Arc<Index>>,
    Query(query): Query<Junk20TickInfoQuery>,
  ) -> Result<Response, ServerError> {
    let token_info = index
      .get_junk20_tokens_info()
      .map_err(|err| ServerError::BadRequest(err.to_string()))?;
    ...
    if query.show_holder.unwrap_or(false) {
      let extended_token_info: Vec<ExtendedTokenInfo> = token_info
        ...
        .map(|info| {
          ...
          let holder = index
            .get_junk20_token_holder(&tick.clone())
            .unwrap_or(Vec::new());
  ...
  async fn june(
    Extension(page_config): Extension<Arc<PageConfig>>,
    Extension(index): Extension<Arc<Index>>,
    Path(DeserializeFromStr(june_query)): Path<DeserializeFromStr<query::June>>,
    Query(query): Query<JsonQuery>,
  ) -> ServerResult<Response> {
    let june = match june_query {
      query::June::SpacedJune(spaced_june) => spaced_june.june,
      query::June::JuneId(june_id) => index
        .get_june_by_id(june_id)?
        .ok_or_not_found(|| format!("june {june_id}"))?,
    };

    let (id, entry) = index.june(june)?.ok_or_else(|| {
      ServerError::NotFound(
        "tracking junes requires index created with `--index-junes` flag".into(),
      )
    })?;
  ...
  async fn junes(
    Extension(page_config): Extension<Arc<PageConfig>>,
    Extension(index): Extension<Arc<Index>>,
  ) -> ServerResult<PageHtml<JunesHtml>> {
    Ok(
      JunesHtml {
        entries: index.junes()?,
      }
  ...
  async fn junes_balances(
    Extension(page_config): Extension<Arc<PageConfig>>,
    Extension(index): Extension<Arc<Index>>,
  ) -> ServerResult<Response> {
    let balances = index.get_june_balance_map()?;
    Ok(
      JuneBalancesHtml { balances }
        .page(page_config)
        .into_response(),
    )
  }

  async fn junes_by_outputs(
    Extension(index): Extension<Arc<Index>>,
    Query(query): Query<OutputsQuery>,
  ) -> ServerResult<Response> {
    let mut all_junes_jsons = Vec::new();
    ...
    for output in outputs {
      ...
      for (june, balances) in junes {
        all_junes_jsons.push(JuneOutputJson { june, balances });
      }
    }

    Ok(Json(all_junes_jsons).into_response())
  }
  ...
  fn search_inner(index: &Index, query: &str) -> ServerResult<Redirect> {
    lazy_static! {
      ...
      static ref JUNE: Regex = Regex::new(r"^[A-Z•.]+$").unwrap();
      static ref JUNE_ID: Regex = Regex::new(r"^[0-9]+:[0-9]+$").unwrap();
    }
    ...
    } else if JUNE.is_match(query) {
      Ok(Redirect::to(&format!("/june/{query}")))
    } else if JUNE_ID.is_match(query) {
      let id = query
        .parse::<JuneId>()
        .map_err(|err| ServerError::BadRequest(err.to_string()))?;

      let june = index.get_june_by_id(id)?.ok_or_not_found(|| "june ID")?;

      Ok(Redirect::to(&format!("/june/{june}")))
    } else {
  ...
  async fn inscription(
    ...
    let june = index.get_june_by_inscription_id(inscription_id)?;
...
mod tests {
  ...
  use crate::junes::{Junestone, Edict, Etching};
  ...
  #[test]
  fn search_by_query_returns_june() {
    TestServer::new().assert_redirect("/search?query=ABCD", "/june/ABCD");
  }
  ...
  #[test]
  fn search_by_path_returns_june() {
    TestServer::new().assert_redirect("/search/ABCD", "/june/ABCD");
  }

  #[test]
  fn search_by_june_id_returns_june() {
    let server = TestServer::new_with_regtest_with_index_junes();

    server.mine_blocks(1);

    let june = June(u128::from(21_000_000 * COIN_VALUE));

    server.assert_response_regex(format!("/june/{june}"), StatusCode::NOT_FOUND, ".*");
    ...
    server.assert_redirect("/search/2/1", "/june/NVTDIJZYIPU");
    server.assert_redirect("/search?query=2/1", "/june/NVTDIJZYIPU");

    server.assert_response_regex("/june/100/200", StatusCode::NOT_FOUND, ".*");
...
<ol start=1 reversed class=blocks>
  ...
  fn nav_displays_chain() {
    TestServer::new().assert_response_regex(
      "/",
      StatusCode::OK,
      ".*<a href=/>Junes<sup>regtest</sup></a>.*",
    );
  }
