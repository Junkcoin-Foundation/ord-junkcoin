...
impl Send {
  pub(crate) fn run(self, options: Options) -> SubcommandResult {
    ...
    let satpoint = match self.outgoing {
      ...
      Outgoing::June { decimal, june } => {
        let transaction = Self::send_junes(
          address,
          &client,
          decimal,
          self.fee_rate,
  ...
  fn send_junes(
    ...
    spaced_june: SpacedJune,
    dunic_outputs: BTreeSet<OutPoint>,
    unspent_outputs: BTreeMap<OutPoint, Amount>,
  ) -> Result<Txid> {
    ensure!(
      index.has_june_index(),
      "sending junes with `ord send` requires index created with `--index-junes` flag",
    );

    let (id, entry) = index
        .june(spaced_june.june)?
        .with_context(|| format!("june `{}` has not been etched", spaced_june.june))?;
    ...
    for output in dunic_outputs {
      ...
      let balance = index.get_june_balance(output, id)?;
    ...
    let unfunded_transaction = Transaction {
      ...
      output: vec![
        TxOut {
          script_pubkey: junestone.encipher(),
          value: 0,
        },
        TxOut {
